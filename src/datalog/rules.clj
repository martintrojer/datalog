...
(defn display-rule
  ...
  [rule]
  (list* '<-
         (-> rule :head literals/display-literal)
         (map literals/display-literal (:body rule))))

(defn display-query
  "Return a query in a readable format."
  [query]
  (list* '?- (literals/display-literal query)))
...
(defn is-safe?
  ...
  [rule]
  (let [hv (literals/literal-vars (:head rule))
        bpv (apply set/union (map literals/positive-vars (:body rule)))
        bnv (apply set/union (map literals/negative-vars (:body rule)))
        ehv (set/difference hv bpv)
        env (set/difference bnv bpv)]
    (when-not (empty? ehv)
...
(defmacro <-
  ...
  [hd & body]
  (let [head (literals/build-atom hd :datalog.literals/literal)
        body (map literals/build-literal body)]
...
(defmacro ?-
  ...
  [& q]
  (let [qq (literals/build-atom q :datalog.literals/literal)]
...
(defn compute-sip
  ...
  (let [next-lit (fn [bv body]
                   (or (first (drop-while
                               #(not (literals/literal-appropriate? bv %))
                               body))
                       (first (drop-while (complement literals/positive?) body))))
        adorn (fn [lit bvs]
                (if (i-preds (literals/literal-predicate lit))
                  (let [bnds (set/union (literals/get-cs-from-vs lit bvs)
                                    (literals/get-self-bound-cs lit))]
                    (literals/adorned-literal lit bnds))
                  lit))
        new-h (literals/adorned-literal (:head rule) bindings)]
    (loop [bound-vars (literals/get-vs-from-cs (:head rule) bindings)
           body (:body rule)
           sip []]
      (if-let [next (next-lit bound-vars body)]
        (recur (set/union bound-vars (literals/literal-vars next))
               (remove #(= % next) body)
...
(defn predicate-map
  ...
  [rs]
  (let [add-rule (fn [m r]
                   (let [pred (-> r :head literals/literal-predicate)
                         os (get m pred #{})]
...
(defn all-predicates
  ...
  [rs]
  (set (map literals/literal-predicate (map :head rs))))
...
(defn non-base-rules
  ...
  (let [pred (all-predicates rs)
        non-base (fn [r]
                   (if (some #(pred %)
                             (map literals/literal-predicate (:body r)))
                     r
                     nil))]
...
(defn apply-rule
  ...
  ([db-1 db-2 rule]
     ...
     (let [head (:head rule)
           body (:body rule)
           step (fn [bs lit]
                  (util/trace-datalog (println bs)
                                      (println lit))
                  (literals/join-literal db-1 lit bs))
           bs (reduce step empty-bindings body)]
       (do (util/trace-datalog (println bs))
           (literals/project-literal db-2 head bs)))))
